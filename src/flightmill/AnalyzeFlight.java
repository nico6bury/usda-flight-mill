 ...
 * @author wjrfo
 */
public class AnalyzeFlight {

    private double pathDistance = 1.0996;   // should be updated by config
    private double gapTime = 20.0;          // should be updated by config
    List<DataLine> speedList = new ArrayList<>();
    int channelCnt = 0;
    ...
    public static void main(String[] args) {
        try {
            ...
            List<DataLine> standardList = standardizePeaks.getOutputList();
            AnalyzeFlight af = new AnalyzeFlight(standardList);
    ...
    public AnalyzeFlight() {
        
    }
    
    public AnalyzeFlight(List<DataLine> inputList) {
        
//        Config config = Config.loadConfig();
        
        channelCnt = inputList.get(0).channels.length;
        
        speedList = makeSpeedList(inputList);
    }
    
    private class SortByChannel implements Comparator<DataLine>{


        @Override
        public int compare(DataLine arg0, DataLine arg1) {
            int cCnt = arg0.channels.length;
            
            for (int idx = 0; idx < cCnt; idx++) {
                if (arg0.channels[idx] > arg1.channels[idx]) {
                    return 1;
                }
                if (arg0.channels[idx] < arg1.channels[idx]) {
                    return -1;
    ...
    List<DataLine> makeSpeedList(List<DataLine> inputList) {
        
        List<DataLine> speedList = new ArrayList<DataLine>();
        
        double[] lastEntry = inputList.get(0).channels;
        for (DataLine line : inputList) {
            for (int idx = 0; idx < lastEntry.length; idx++) {
                if (line.channels[idx] > 0.5 && lastEntry[idx] < 0.5) {
                    System.out.printf("%5.2f, %2.1f, %2.1f\n", line.time, line.channels[0], line.channels[1]);
                    speedList.add(line);
                    break;
                }
            }
            lastEntry = line.channels;
        }
        System.out.println("speedList length " + speedList.size());
        return speedList;
    }
